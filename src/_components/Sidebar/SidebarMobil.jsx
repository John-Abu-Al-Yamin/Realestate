...
const SidebarMobile = () => {
  ...
  const navItems = [
    ...
    { key: "users", href: "/users", icon: UserRound },
  ...
  const renderNavLink = (item) => {
    ...
    return (
      ...
      >
        ...
        <span className="font-medium whitespace-nowrap">{t(`sidebar.${item.key}`)}</span>
      </NavLink>
    );
  };
  ...
  return (
    <>
      ...
      >
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {isActive && <span className="text-xs font-medium">{t(`sidebar.${item.key}`)}</span>}
              </div>
            </NavLink>
          );
        })}
        ...
        >
          <ChevronUp size={20} className={`${isExpanded ? "rotate-180" : ""} transition-transform`} />
          {isExpanded && <span className="text-xs font-medium ml-2">{t("sidebar.more")}</span>}
        </button>
      </div>
